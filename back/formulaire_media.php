...
if (!empty($_POST)) {
    ...
    if (!$error) { //insert
        ...
        $picture_bdd = BASE_PATH . 'assets/upload/'  . uniqid() . date_format(new DateTime(), 'd_m_Y_H_i_s') . $_FILES['title_media']['name'];
      
       $picture_base='assets/upload/'. uniqid() . date_format(new DateTime(), 'd_m_Y_H_i_s') . $_FILES['title_media']['name'];
      //debug($picture_bdd);
      // die;
        ...
        $resultat = execute("INSERT INTO media (title_media,name_media,id_media_type) VALUES (:title_media,:name_media,:id_media_type)", array(
            //condition ternaire
            ':title_media' => !empty($_FILES['title_media']['name']) ? $picture_base : $_POST['title_media'],
