...
if (!empty($_GET) && isset($_GET['id'])) {
    ...
    $supprfait = execute(
        "DELETE FROM comment WHERE id_comment=:id",
        array(
            ':id' => $_GET['id']
    ...
    <tbody>
        <?php foreach ($comments as $comment) : ?>
            ...
            <tr>
                ...
                <td class="comment-photo"><img width="90" src="<?= BASE_PATH . $comment['title_media']; ?>" alt="<?= $comment['name_media']  ?>"></td>
                <td><?= $comment['activate']; ?></td>
                <td> <a href="?id=<?= $comment['id_comment']; ?>" onclick="return confirm('Etes-vous sûr?')" class="btn btn-outline-danger">Supprimer</a></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
</table>
